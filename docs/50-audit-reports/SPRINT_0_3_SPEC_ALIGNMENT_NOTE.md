...
**Security Impact.** The exposed SQL API surface is reduced by removing publicly executable authorization helpers. Authorization decisions remain entirely within PostgreSQL RLS, reducing the risk of accidental or unauthorized invocation and simplifying the application's trust boundary. PostgREST cannot enumerate or invoke `private.*` functions, so the role-check helper is unreachable from any client credential (anon or authenticated), and a compromised or misused client token cannot probe role membership through the Data API.
