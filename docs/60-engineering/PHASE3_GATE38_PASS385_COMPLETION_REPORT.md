...
**Pass:** 3.8.5 / 3.8.5B / 3.8.5C / 3.8.5D / 3.8.5E — Tenant readiness evaluator, snapshot persistence and guarded activation
...
| `20260727162823_eaaf747f-4c52-4ba3-9fb6-2d6dbb583817.sql` | Pass 3.8.5E (append-only): **evaluator volatility correction** — `private.fn_onboarding_evaluate_readiness_json(uuid, text)` and `private.fn_onboarding_evaluate_readiness_present_json(uuid, text)` are now explicitly `VOLATILE`, because the envelope embeds the lock-sensitive `no_concurrent_activation` check and must never be planner-cached as `STABLE`. Response fields and readiness semantics are unchanged. Also replaced `private.fn_setting_value_invalid_reason` with **strict JSON metadata typing**: `required` must be a JSON boolean, `min`/`max` JSON numbers, `regex` a JSON string, `enum` a non-empty array of JSON strings; an enum data type without a valid option list, an unknown data type, `min > max`, and a malformed or unsupported regex all yield the bounded `invalid_schema` token. Coercions such as `{"required":"true"}`, `{"required":1}`, `{"min":"5"}` and `{"max":"100"}` are rejected. Raw PostgreSQL exception text is never surfaced. |

No migration file was rewritten; all five remain in chronological order.
