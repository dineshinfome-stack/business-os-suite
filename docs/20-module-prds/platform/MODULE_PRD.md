...
related_engines: ["ENG-001", "ENG-002", "ENG-003", "ENG-004", "ENG-005", "ENG-006", "ENG-020", "ENG-024", "ENG-021", "ENG-023", "ENG-025"]
related_adrs: ["ADR-011", "ADR-032", "ADR-014"]
related_modules: ["MOD-002", "MOD-003", "MOD-004", "MOD-005", "MOD-006", "MOD-007", "MOD-008", "MOD-009", "MOD-010", "MOD-011", "MOD-012", "MOD-013", "MOD-014", "MOD-015", "MOD-016", "MOD-017", "MOD-018"]
referenced_by: []
tags: ["module", "prd"]
document_type: "Module PRD"
---

# MOD-001 — Platform Administration Module PRD

> **Authoritative business specification** for the **Platform Administration (MOD-001)** bounded context. This document consumes — and MUST NOT redefine — Foundation, Architecture, ERP Core Engines (`ENG-NNN`), and Accepted ADRs (`ADR-NNN`). Cross-module references use `MOD-NNN`. On any conflict with an upstream authoritative document, the upstream wins and this PRD is corrected in the same change.

## 1. Module Overview

**Purpose.** Tenant, company, branch, financial-year, user, role, and platform-wide configuration surface.

**Business Objectives.**

- Provide the authoritative business surface for the Platform bounded context.
...
- Own the master data and transactions listed in sections 5–6 for the Platform bounded context.
...
- Financial year open and close
- User invitation and deactivation
- Role grant and revocation
- Configuration change with audit
...
**Ownership.** All entities above are owned by **Platform Administration (MOD-001)** unless explicitly declared as shared in section 13.
...
- A company cannot be deleted while it has open financial years.
- A user cannot self-revoke the last platform-admin role in a tenant.
- Configuration changes are always tenant-scoped and versioned.
...
No engine behavior may be redefined. Consumption declared here is authoritative for **Platform Administration (MOD-001)**.
