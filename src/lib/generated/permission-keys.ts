/**
 * AUTO-GENERATED — do not edit by hand.
 * Regenerate with `bun run gen:permissions` after editing
 * `docs/15-governance/permission-catalog.manifest.yaml`.
 */

export const PERMISSIONS = {
  PLATFORM_USERS_VIEW: "platform.users.view",
  PLATFORM_USERS_CREATE: "platform.users.create",
  PLATFORM_USERS_UPDATE: "platform.users.update",
  PLATFORM_USERS_DELETE: "platform.users.delete",
  PLATFORM_ORGANIZATIONS_VIEW: "platform.organizations.view",
  PLATFORM_ORGANIZATIONS_MANAGE: "platform.organizations.manage",
  PLATFORM_ROLES_VIEW: "platform.roles.view",
  PLATFORM_ROLES_ASSIGN: "platform.roles.assign",
  PLATFORM_AUDIT_VIEW: "platform.audit.view",
  SETTINGS_GENERAL_VIEW: "settings.general.view",
  SETTINGS_GENERAL_UPDATE: "settings.general.update",
  SETTINGS_SECURITY_MANAGE: "settings.security.manage",
  PLATFORM_SETTINGS_MANAGE: "platform.settings.manage",
  AUDIT_LOGS_VIEW: "audit.logs.view",
  NOTIFICATIONS_INBOX_READ: "notifications.inbox.read",
  NOTIFICATIONS_INBOX_CREATE: "notifications.inbox.create",
  NOTIFICATIONS_INBOX_MANAGE: "notifications.inbox.manage",
  SEARCH_GLOBAL_USE: "search.global.use",
  SEARCH_HISTORY_MANAGE: "search.history.manage",
  WORKSPACE_WORKSPACE_READ: "workspace.workspace.read",
  WORKSPACE_WORKSPACE_MANAGE: "workspace.workspace.manage",
  WORKSPACE_ORGANIZATION_READ: "workspace.organization.read",
  WORKSPACE_ORGANIZATION_UPDATE: "workspace.organization.update",
  WORKSPACE_BRANDING_READ: "workspace.branding.read",
  WORKSPACE_BRANDING_UPDATE: "workspace.branding.update",
  WORKSPACE_PROFILE_READ: "workspace.profile.read",
  WORKSPACE_PROFILE_UPDATE: "workspace.profile.update",
  WORKSPACE_MEMBER_READ: "workspace.member.read",
  WORKSPACE_MEMBER_INVITE: "workspace.member.invite",
  WORKSPACE_MEMBER_REMOVE: "workspace.member.remove",
  WORKSPACE_INVITATION_READ: "workspace.invitation.read",
  WORKSPACE_INVITATION_REVOKE: "workspace.invitation.revoke",
  PLATFORM_TENANT_READ: "platform.tenant.read",
  PLATFORM_TENANT_CREATE: "platform.tenant.create",
  PLATFORM_TENANT_UPDATE: "platform.tenant.update",
  PLATFORM_TENANT_ACTIVATE: "platform.tenant.activate",
  PLATFORM_TENANT_SUSPEND: "platform.tenant.suspend",
  PLATFORM_TENANT_ARCHIVE: "platform.tenant.archive",
  PLATFORM_TENANT_MAINTENANCE: "platform.tenant.maintenance",
  PLATFORM_TENANT_RESTORE: "platform.tenant.restore",
  PLATFORM_TENANT_DELETE_SCHEDULE: "platform.tenant.delete_schedule",
  PLATFORM_TENANT_DELETE: "platform.tenant.delete",
  PLATFORM_COMPANY_READ: "platform.company.read",
  PLATFORM_COMPANY_CREATE: "platform.company.create",
  PLATFORM_COMPANY_ACTIVATE: "platform.company.activate",
  PLATFORM_COMPANY_DEACTIVATE: "platform.company.deactivate",
  PLATFORM_COMPANY_ARCHIVE: "platform.company.archive",
  PLATFORM_COMPANY_SET_DEFAULT: "platform.company.set_default",
  PLATFORM_BRANCH_READ: "platform.branch.read",
  PLATFORM_BRANCH_CREATE: "platform.branch.create",
  PLATFORM_BRANCH_UPDATE: "platform.branch.update",
  PLATFORM_BRANCH_ARCHIVE: "platform.branch.archive",
  PLATFORM_BRANCH_SET_DEFAULT: "platform.branch.set_default",
  PLATFORM_FINANCIAL_YEAR_READ: "platform.financial_year.read",
  PLATFORM_FINANCIAL_YEAR_CREATE: "platform.financial_year.create",
  PLATFORM_FINANCIAL_YEAR_OPEN: "platform.financial_year.open",
  PLATFORM_FINANCIAL_YEAR_CLOSE: "platform.financial_year.close",
  PLATFORM_FINANCIAL_YEAR_ARCHIVE: "platform.financial_year.archive",
  PLATFORM_FINANCIAL_YEAR_SET_DEFAULT: "platform.financial_year.set_default",
  PLATFORM_DASHBOARD_VIEW: "platform.dashboard.view",
  PLATFORM_IDENTITY_DASHBOARD_VIEW: "platform.identity_dashboard.view",
  PLATFORM_USERS_SUSPEND: "platform.users.suspend",
  PLATFORM_USERS_ACTIVATE: "platform.users.activate",
  PLATFORM_USERS_ARCHIVE: "platform.users.archive",
  PLATFORM_USERS_RESTORE: "platform.users.restore",
  PLATFORM_USERS_RESET_PASSWORD: "platform.users.reset_password",
  PLATFORM_USERS_FORCE_RESET: "platform.users.force_reset",
  PLATFORM_USERS_LOCK: "platform.users.lock",
  PLATFORM_USERS_UNLOCK: "platform.users.unlock",
  PLATFORM_USERS_INVITE: "platform.users.invite",
  PLATFORM_ROLES_CREATE: "platform.roles.create",
  PLATFORM_ROLES_UPDATE: "platform.roles.update",
  PLATFORM_ROLES_CLONE: "platform.roles.clone",
  PLATFORM_ROLES_ARCHIVE: "platform.roles.archive",
  PLATFORM_ROLES_DELETE: "platform.roles.delete",
  PLATFORM_PERMISSIONS_VIEW: "platform.permissions.view",
  PLATFORM_MEMBERSHIPS_MANAGE: "platform.memberships.manage",
  PLATFORM_POLICIES_VIEW: "platform.policies.view",
  PLATFORM_POLICIES_MANAGE: "platform.policies.manage",
  PLATFORM_INVITATIONS_VIEW: "platform.invitations.view",
  PLATFORM_INVITATIONS_MANAGE: "platform.invitations.manage",
} as const;

export type PermissionKey = (typeof PERMISSIONS)[keyof typeof PERMISSIONS];

export const ALL_PERMISSION_KEYS: readonly PermissionKey[] = [
  "platform.users.view",
  "platform.users.create",
  "platform.users.update",
  "platform.users.delete",
  "platform.organizations.view",
  "platform.organizations.manage",
  "platform.roles.view",
  "platform.roles.assign",
  "platform.audit.view",
  "settings.general.view",
  "settings.general.update",
  "settings.security.manage",
  "platform.settings.manage",
  "audit.logs.view",
  "notifications.inbox.read",
  "notifications.inbox.create",
  "notifications.inbox.manage",
  "search.global.use",
  "search.history.manage",
  "workspace.workspace.read",
  "workspace.workspace.manage",
  "workspace.organization.read",
  "workspace.organization.update",
  "workspace.branding.read",
  "workspace.branding.update",
  "workspace.profile.read",
  "workspace.profile.update",
  "workspace.member.read",
  "workspace.member.invite",
  "workspace.member.remove",
  "workspace.invitation.read",
  "workspace.invitation.revoke",
  "platform.tenant.read",
  "platform.tenant.create",
  "platform.tenant.update",
  "platform.tenant.activate",
  "platform.tenant.suspend",
  "platform.tenant.archive",
  "platform.tenant.maintenance",
  "platform.tenant.restore",
  "platform.tenant.delete_schedule",
  "platform.tenant.delete",
  "platform.company.read",
  "platform.company.create",
  "platform.company.activate",
  "platform.company.deactivate",
  "platform.company.archive",
  "platform.company.set_default",
  "platform.branch.read",
  "platform.branch.create",
  "platform.branch.update",
  "platform.branch.archive",
  "platform.branch.set_default",
  "platform.financial_year.read",
  "platform.financial_year.create",
  "platform.financial_year.open",
  "platform.financial_year.close",
  "platform.financial_year.archive",
  "platform.financial_year.set_default",
  "platform.dashboard.view",
  "platform.identity_dashboard.view",
  "platform.users.suspend",
  "platform.users.activate",
  "platform.users.archive",
  "platform.users.restore",
  "platform.users.reset_password",
  "platform.users.force_reset",
  "platform.users.lock",
  "platform.users.unlock",
  "platform.users.invite",
  "platform.roles.create",
  "platform.roles.update",
  "platform.roles.clone",
  "platform.roles.archive",
  "platform.roles.delete",
  "platform.permissions.view",
  "platform.memberships.manage",
  "platform.policies.view",
  "platform.policies.manage",
  "platform.invitations.view",
  "platform.invitations.manage",
] as const;

export const ROLE_KEYS = {
  PLATFORM_OWNER: "platform_owner",
  PLATFORM_ADMIN: "platform_admin",
  ORG_OWNER: "org_owner",
  ADMINISTRATOR: "administrator",
  MANAGER: "manager",
  SUPERVISOR: "supervisor",
  EMPLOYEE: "employee",
  READ_ONLY: "read_only",
} as const;

export type RoleKey = (typeof ROLE_KEYS)[keyof typeof ROLE_KEYS];
