...
import { tenantKeys } from "@/lib/tenant/query-keys";
import {
  createInvitation,
  getMyProfile,
...
} from "@/lib/tenant/business-functions";
import type { InvitationRow, OrgMemberDirectoryRow, OrgRole } from "@/lib/tenant/business-types";

export const Route = createFileRoute("/_authenticated/tenant")({
  head: () => ({
    meta: [
      { title: `Tenant — ${APP_NAME}` },
      {
        name: "description",
        content: `${APP_NAME} — tenant profile, branding, team directory, and invitations.`,
      },
    ],
  }),
...
function TenantPage() {
  ...
  return (
    <PageContainer
      ...
      description="Manage your tenant's identity, branding, team, and invitations."
    >
      {!organizationId ? (
        <EmptyState
          title="No active tenant"
          description="Select a tenant from the top bar to manage its settings."
        />
      ) : (
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="profile">Tenant profile</TabsTrigger>
...
function OverviewTab() {
  ...
  const members = useQuery({
    queryKey: tenantKeys.members(organizationId),
    queryFn: () => listOrgMembers({ data: {} }),
    enabled: !!organizationId,
  });
  const invites = useQuery({
    queryKey: tenantKeys.invitations(organizationId),
    queryFn: () => listInvitations({ data: {} }),
    enabled: !!organizationId,
  });
  const pendingCount = (invites.data ?? []).filter((i: InvitationRow) => i.status === "pending").length;
...
function OrgProfileTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: tenantKeys.profile(organizationId),
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: tenantKeys.profile(organizationId) });
    },
...
function BrandingTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: tenantKeys.branding(organizationId),
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: tenantKeys.branding(organizationId) });
    },
    onError: (e) => notify.error(e instanceof Error ? e.message : "Save failed"),
  });
...
function MyProfileTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: tenantKeys.myProfile(organizationId),
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: tenantKeys.myProfile(organizationId) });
      void qc.invalidateQueries({ queryKey: tenantKeys.members(organizationId) });
    },
    onError: (e) => notify.error(e instanceof Error ? e.message : "Save failed"),
  });
  if (isLoading) return <Skeleton className="h-64 w-full" />;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>How you appear inside this tenant.</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-4 md:grid-cols-2">
        <Field label="Display name" value={merged.displayName} onChange={set("displayName")} />
    ...
    <Card>
      <CardContent className="p-0">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map((m: OrgMemberDirectoryRow) => (
...
function InvitationsTab() {
  ...
  const createM = useMutation({
    ...
    onSuccess: (res: { token: string }) => {
      const url = `${window.location.origin}/tenant/accept?token=${encodeURIComponent(res.token)}`;
      setLastLink(url);
      setEmail("");
      notify.success("Invitation created — copy the link below to share it.");
      void qc.invalidateQueries({ queryKey: tenantKeys.invitations(organizationId) });
    },
