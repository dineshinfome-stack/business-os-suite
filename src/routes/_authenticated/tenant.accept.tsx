...
import { acceptInvitation } from "@/lib/tenant/business-functions";

const search = z.object({ token: z.string().min(20).max(200).optional() });

export const Route = createFileRoute("/_authenticated/tenant/accept")({
...
function AcceptPage() {
  const { token } = useSearch({ from: "/_authenticated/tenant/accept" });
